...
class BankUi:
    def __init__(self,root):
        self.root = root
        self.header = Label(self.root,text="UNIQUE BANK",bg="#964B00",fg="white",font=("arial",40,"bold"))
        self.header.pack(fill=X)
        self.frame = Frame(self.root,bg="#FFFDD0",width=5000,height=1000)
        root.geometry("800x500")
        self.button1 = Button(self.frame,text="Click to begin transactions",bg="#964B00",fg="white",font=ARIAL,command = self.begin_page)
        self.q = Button(self.frame, text="Quit", bg="#964B00", fg="white", font=ARIAL, command=self.root.destroy)
        self.q.place(x=650, y=340, width=200, height=40)
        self.button1.place(x=500,y=230,width=500,height=50)
        self.countter = 2
        self.frame.pack()
    ...
    def withdraw_money_page(self):
        ...
        self.button = Button(self.frame,text="Verify Face Id to enter your password",bg="#50A8B0",fg="white",font=ARIAL,command=self.video_check)
        self.q = Button(self.frame,text="Quit",bg="#50A8B0",fg="white",font=ARIAL,command = self.root.destroy)
    ...
    def password_verification(self):
        ...
        self.q = Button(self.frame,text="Quit",bg="#50A8B0",fg="white",font=ARIAL,command = self.root.destroy)
        self.b = Button(self.frame,text="Back",bg="#50A8B0",fg="white",font=ARIAL,command = self.begin_page)
        self.plabel.place(x=125,y=160,width=250,height=20)
    ...
    def video_check(self):
        ...
        try:
            ...
            if Counter(real_user_list).most_common(1)[0][0] == 'unknown':
                ...
                messagebox._show("Verification Info!", "Face Id detected.Now enter password!")
